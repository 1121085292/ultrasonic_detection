...
bool UltrasonicComponent::Proc(const std::shared_ptr<EchoList> &echo_list,
  ...
  for (const auto &echo : echo_list->echo_list()) {
    ...
    MinFilter min_filter(MinFilterParams().window_size,
                         MinFilterParams().threshold);
  ...
  for (int sensor_id = 0; sensor_id < 12; ++sensor_id) {
    ...
    if (use_triangle_measured_) {
      ...
      if (sensor_id == 1 || sensor_id == 2 || sensor_id == 3 ||
        ...
        auto ur =
            Ultrasonic(coordinate_map_[sensor_id + 1], dis_map[sensor_id][0]);
  ...
  if (now - echo_list->timestamp() < 1) {
    ...
    ultrasonic_list->set_error_code(
        common_msgs::error_code::ErrorCode::PERCEPTION_ERROR);
  }
  // 空间车位信息
