...
void ParkingSpotDetection::ParkingSpotSearch(
    ...
    const ParkingSpotParams& parking_spot_params, const CurbParams& curb_params,
  ...
  std::vector<LineSegment> fit_lines =
      line_segment_ptr_->FitLineSegments(points_, line_fit_params);
  // 1.潜在车位搜索
  std::map<int, std::vector<LineSegment>> line_segments;
  if (!FindPotentialParkingSpots(fit_lines, pose, parking_spot_params,
                                 curb_params, line_segments)) {
    return;
  }
  // 2.对存在潜在车位的线段对再判断
  for (const auto& pair : line_segments) {
    // 满足车位再判断的线段对
    if (!ReEvaluateParallelParkingSpot(pair, parking_spot_params, pose))
      continue;
...
bool ParkingSpotDetection::FindPotentialParkingSpots(
  ...
  for (size_t i = 0; i < fit_lines.size() - 1; ++i) {
    ...
    if (line1.Length() < parking_spot_params.min_line_length ||
        line2.Length() < parking_spot_params.min_line_length)
      continue;
    // 线段近端点组成新的线段
...
bool ParkingSpotDetection::ReEvaluateParallelParkingSpot(
  ...
  // 生长后线段的投影累计长度
  double proj_length1 =
      GrowLineSegment(line1, key, true, parking_spot_params, pose);
  double proj_length2 =
      GrowLineSegment(line2, key, false, parking_spot_params, pose);
...
bool ParkingSpotDetection::CheckParkingSpotConstraints(
  ...
  if (curb_lines_.empty()) {
    ...
    // 横向偏移
    offset =
        std::max(offset, LineSegment().DistanceToLine(line1.GetStart(), line2));
    offset =
        std::max(offset, LineSegment().DistanceToLine(line1.GetEnd(), line2));
    offset =
        std::max(offset, LineSegment().DistanceToLine(line2.GetStart(), line1));
    offset =
        std::max(offset, LineSegment().DistanceToLine(line2.GetStart(), line1));
    // 偏移角度
    angle = line1.AngleBetweenLines(line2);
  }
  if (offset > parking_spot_params.offset_threshold &&
      angle > parking_spot_params.angle_threshold * M_PI / 180) {
    return false;
  }
...
void ParkingSpotDetection::CalculateParkingVertices(double angle,
                                                    ParkingSpot& parking_spot) {
  // 方向向量
  ...
  // 边缘线段在方向向量上投影距离
  LineSegment line(parking_spot_pair_[0].GetEnd(),
                   parking_spot_pair_[1].GetStart());
  ...
  if (parking_spot_type_ == ParkingSpotType::PARALLEL_PARKING) {
    ...
    // 0号点
    spot_right_top =
        spot_left_top + dir * ParkingSpotParams().spot_length_threshold;
    // 2号点
    spot_left_down =
        spot_left_top + top_down_dir * ParkingSpotParams().spot_width_threshold;
    // 3号点
    spot_right_down = spot_right_top +
                      top_down_dir * ParkingSpotParams().spot_width_threshold;
  } else if (parking_spot_type_ == ParkingSpotType::VERTICAL_PLOT) {
    // 1号点以line1的终点为基准+delta
    ...
    // 0号点
    spot_right_top =
        spot_left_top + dir * ParkingSpotParams().spot_width_threshold;
    // 2号点
    spot_left_down = spot_left_top +
                     top_down_dir * ParkingSpotParams().spot_length_threshold;
    // 3号点
    spot_right_down = spot_right_top +
                      top_down_dir * ParkingSpotParams().spot_length_threshold;
  }
...
double ParkingSpotDetection::GrowLineSegment(
  ...
  if (flag) {
    ...
    while (1) {
      ...
      // 判断两条拟合线段距离是否满足阈值
      if ((line_bak.GetStart() - pre_line.GetEnd()).Length() >
          parking_spot_params.grow_line_dist) {
        return project_length;
        // 判断两条拟合线段夹角是否满足阈值
      } else if (line_bak.AngleBetweenLines(pre_line) >
                 parking_spot_params.grow_line_dist) {
        return project_length;
      } else {
        // 生长
    ...
    while (1) {
      ...
      // 判断两条拟合线段距离是否满足阈值
      if ((next_line.GetStart() - line_bak.GetEnd()).Length() >
          parking_spot_params.grow_line_dist) {
        return project_length;
        // 判断两条拟合线段夹角是否满足阈值
      } else if (next_line.AngleBetweenLines(line_bak) >
                 parking_spot_params.grow_line_angle * M_PI / 180) {
        return project_length;
      } else {
        // 生长
...
bool ParkingSpotDetection::IsCurbLine(const LineSegment& line,
