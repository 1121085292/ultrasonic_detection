...
class ParkingSpotDetection {
 public:
  // 空间车位搜索
  void ParkingSpotSearch(const Point2D& point,
 ...
 private:
  ...
  // 生长线段
  double GrowLineSegment(const LineSegment& line, int key, bool flag,
                         const ParkingSpotParams& parking_spot_params,
                         const std::shared_ptr<Pose>& pose);

  // 判断是否为路牙特征线段
  bool IsCurbLine(const LineSegment& line, const CurbParams& curb_params,
                  const std::shared_ptr<Pose>& pose);
