...
void LineSegment::Init(const LineFitParams& line_fit_params) {
  if (!is_init_) {
    ...
    line_fit_params_.cluster_dist_threshold =
        line_fit_params.cluster_dist_threshold;
    line_fit_params_.merge_angle_threshold =
        line_fit_params.merge_angle_threshold;
    line_fit_params_.merge_dist_threshold =
        line_fit_params.merge_dist_threshold;
...
std::vector<LineSegment> LineSegment::FitLineSegments(
  ...
  if (!clusters.empty()) {
    for (const auto& cluster : clusters) {
      ...
      // 将合并后的线段添加到结果
      segments.insert(segments.end(), divided_segments.begin(),
                      divided_segments.end());
    }
  }
...
std::vector<Cluster> LineSegment::ClusterPoints(
...
// 递归划分线段
void LineSegment::DivideLineSegments(std::vector<LineSegment>& segments,
                                     const std::vector<Point2D>& points) {
  ...
  for (const auto& segment : segments) {
    ...
    if (max_dist > line_fit_params_.merge_dist_threshold &&
        static_cast<int>(split_idx) != -1) {
...
void LineSegment::MergeLineSegments(std::vector<LineSegment>& segments) {
  ...
  for (size_t i = 0; i < segments.size(); i++) {
    if (i == segments.size() - 1 ||
        fabs(LineAngle(segments[i]) - LineAngle(segments[i + 1])) >
            line_fit_params_.merge_angle_threshold) {
